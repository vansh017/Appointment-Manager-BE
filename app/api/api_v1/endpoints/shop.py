...
from services.shop_service import _create_shop, _get_shops, _get_shop_menu, _create_shop_menu
...
@shop.post("/menu/create", name="Create shop",
           description="Creating shop")
def api_authenticate_user(*, request: Request,
                          user_id: int = Query(default=None),
                          db: Session = Depends(get_db),
                          menu_details : List[CatalogSchema]):
    """
    Login route for user ... sends otp on user's email
    the user will be marked as in active when he/she enters wrong password multiple times
    :param menu_details:
    ...
    """
    try:
        response = _create_shop_menu(db=db,request=request,user_id=user_id,menu_details=menu_details)
