...
from services.customer_service import _add_customer_queue, _get_customer_queue, _update_customer_queue
...
async def update_queue_realtime(db:Session, request: Request, shop_id : int, user_id: int):
    try:
        updated_queue = await _get_customer_queue(db=db, request=request, shop_id=shop_id,
                                                  user_id=user_id,
                                                  filters=None, sort_fields=None, paginate=None)

        await connection_manager.broadcast_to_shop(updated_queue.data, shop_id=shop_id)
...
@customer.post("/update", name="Update queue",
           description="update Customer queue")
async def create_customer_row(*, request: Request,
                        user_id: int = Query(default=None),
                        customer_data : UpdateStatus = Body(),
                        db: Session = Depends(get_db),
                        ):
    """
    Login route for user ... sends otp on user's email
    ...
    """
    try:
        response = _update_customer_queue(db=db, request=request,customer_data=customer_data,user_id=user_id)

        await update_queue_realtime(db=db, request=request,shop_id=customer_data.shop_id,user_id=user_id )
